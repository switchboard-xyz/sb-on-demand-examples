...
(async function main() {
  ...
  const keypair = await AnchorUtils.initKeypairFromFile(
    "/Users/alexstewart/.config/solana/switchboard_work.json"
  );
  ...
  for (let numFeeds = 1; numFeeds <= 5; numFeeds++) {
    for (let numSignatures = 1; numSignatures <= 5; numSignatures++) {
      console.log(
        `Running test with ${numFeeds} feed(s) and ${numSignatures} signature(s)...`
      );

      // Select subset of feeds for this test iteration
      const selectedFeeds = feeds
        .slice(0, numFeeds)
        .map((pubkey) => new sb.PullFeed(program, pubkey));
      ...
      try {
        ...
        console.error(
          `Error with ${numFeeds} feeds and ${numSignatures} signatures:`,
          error
        );
      }
