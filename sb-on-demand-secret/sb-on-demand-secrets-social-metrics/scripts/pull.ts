...
(async function main() {
  try {
    ...
    // secrets start
    const secretNameExpansionTask = "${BEARER_TOKEN_X}";
    const secretName = "BEARER_TOKEN_X";
    // Pull in API Key from .env file
    const API_KEY = process.env.BEARER_TOKEN_X;
    ...
    const secretFeedJob = [
      buildXJob(secretNameExpansionTask, keypair),
    ];
    const conf: any = {
      name: "Social Metrics number", // the feed name (max 32 bytes)
