...
/**
 * Program artifact paths generated by `anchor build`
 */
export const BASIC_IDL_PATH = "target/idl/basic_oracle_example.json";
...
/**
 * Loads the basic example environment.
 *
 * This prefers explicit env vars in container/CI workflows and falls back to
 * the Solana CLI config file for normal local development.
 */
export async function loadBasicEnv(): Promise<BasicSolanaEnv> {
  const rpcUrl =
    process.env.ANCHOR_PROVIDER_URL ?? process.env.SOLANA_RPC_URL ?? "";
  const keypairPath =
    process.env.ANCHOR_WALLET ?? process.env.SOLANA_KEYPAIR_PATH ?? "";

  if (!rpcUrl || !keypairPath) {
    try {
      const config = await sb.AnchorUtils.loadEnv();
      const pid = config.isMainnet
        ? sb.ON_DEMAND_MAINNET_PID
        : sb.ON_DEMAND_DEVNET_PID;
      const program = config.program ?? (await anchor.Program.at(pid, config.provider));
      ...
      throw new Error(
        "Failed to load Solana config. Set ANCHOR_PROVIDER_URL and ANCHOR_WALLET, or configure the Solana CLI first."
      );
    }
  }

  const keypair = await sb.AnchorUtils.initKeypairFromFile(keypairPath);
  const wallet = new anchor.Wallet(keypair);
  const connection = new anchor.web3.Connection(rpcUrl, TX_CONFIG.commitment);
  const provider = new anchor.AnchorProvider(connection, wallet, TX_CONFIG);
  ...
  const pid = isMainnet ? sb.ON_DEMAND_MAINNET_PID : sb.ON_DEMAND_DEVNET_PID;
...
/**
 * Loads an Anchor program from a local IDL artifact
 *
 * This keeps the example aligned with the source in this repository rather
 * than relying on a deployed on-chain IDL that may be older than local code.
 *
 * @async
 * @param {anchor.Provider} provider - Anchor provider with connection and wallet
 * @param {string} idlPath - Path to the program's IDL JSON file
 * @returns {Promise<anchor.Program>} Initialized Anchor program instance
 *
 * @throws {Error} If the IDL file can't be loaded
 *
 * @example
 * ```typescript
 * const provider = anchor.AnchorProvider.env();
 * const program = await programFromLocalIdl(provider, "./target/idl/my_program.json");
 * ```
 */
export async function programFromLocalIdl(
  provider: anchor.Provider,
  idlPath: string
): Promise<anchor.Program> {
  try {
    const rawIdl = fs.readFileSync(idlPath, "utf8");
    const idl = JSON.parse(rawIdl) as anchor.Idl;
    return new anchor.Program(idl, provider);
  } catch {
    throw new Error(
      `Failed to load local IDL at ${idlPath}. Run \`npm run build\` first.`
    );
  }
}
...
export async function loadBasicProgram(
  ...
  return await programFromLocalIdl(provider, BASIC_IDL_PATH);
}
...
/**
 * Normalizes a feed ID into the bare 64-character hex format expected by the
...
export function normalizeFeedId(feedId: string): string {
  ...
  if (!FEED_ID_REGEX.test(trimmed)) {
    throw new Error(
      "Feed IDs must be 32-byte hex. Pass 64 hex characters, without the 0x prefix."
    );
  }
...
export function logFeedId(feedId: string): void {
  if (!process.argv.includes("--feedId")) {
    console.log("ℹ️  No --feedId flag passed, using the default BTC/USD feed:");
    console.log(`   Feed ID (bare hex): ${feedId}`);
    console.log("   Search for other assets at: https://explorer.switchboardlabs.xyz/");
  } else {
    console.log("Using feed ID (bare hex):", feedId);
  }
}
